...
- 앱 개발
  ...
  - 검색 도구 (§3.3 ReAct, `create_agent`, 최대 반복 7회) [고정]
    ...
    - web_search: 웹 수집(`crawler/web`) — DuckDuckGo+BeautifulSoup, 최근 6개월, 최대 결과 10,  
      지수 백오프 재시도 3회·타임아웃 5초, 403/429는 즉시 중단 후 보고
    - youtube_search: 영상·자막 수집(`crawler/youtube`) — YouTube Data API(최근 6개월·조회수 1,000 이상·  
      최대 결과 10) + YouTubeLoader 자막, 쿼터 초과 시 캐시 반환, 자막 없는 영상은 스킵 후 로그
  ...
  - 응답 합성·출력 스키마 (§3.1 Structured Output) [고정] — 프론트엔드가 소비하는 최종 계약
    - `query`: 원 질의 에코(문자열)
    - `summary`: 핵심 요약(문자열)
    - `code_examples[]`: `{lang, code, explain, source}` — `source`는 코드 청크 참조로  
      `code_search` 결과는 `indexing-code.md`의 `chunk_id`(`{상대경로}#{symbol}#{start_line}`)를 그대로 사용
    - `sources[]`: `{type(textbook|code|web|youtube), ref|url, score, fetched_at?}` —  
      교재·코드는 엔티티/청크 `ref`, 웹·영상은 `url`+`fetched_at`, 전 소스 `score`(관련도·신뢰도) 표기
    - `missing_sources[]`: 타임아웃·실패로 미도달한 소스 유형 배열
    - `used_models`: 태스크별 선정 모델 맵(예: `{routing, code_synthesis}`)
    - 근거 없는 내용 생성 금지(환각 차단)
  - API 계층 [기준] — FastAPI 기본(비동기·스트리밍 적합), 변경 필요 시 사용자 문의
    - `POST /search` 요청 스키마 [고정]: `query`(필수, 학습자 질문),  
      `llm`(선택, `claude`|`openai`|`gemini` 합성 모델 힌트 — 미지정 시 §3.2 태스크별 자동 라우팅)
    - `POST /search` 응답 [고정]: 부분 결과 SSE 스트리밍 → 최종 Structured Output 반환.  
      SSE 이벤트는 최종 스키마 필드와 정합 — `summary`·`code`·`source`·`missing`·`done`  
      (`done` data에 최종 Structured Output 전체 포함), 각 event data는 최종 스키마 필드명을 그대로 사용
    - `GET /health`: 상태 점검
    - 실행 방식 [고정]: UI 스트리밍+병렬 도구 호출이므로 LCEL 비동기 스트리밍(astream) 사용
    - 위 요청·SSE·최종 스키마가 프론트엔드(`prompts/frontend.md`)가 소비하는 API 계약의 정본임
  - 세션·기술 요구사항
...
- MUST: `.env`로 키 관리, Config·소스 분리. 코드 참조 키명은 `.env` 실제 변수명과 대조 후 사용  
  (예: Cohere 키가 오탈자 변수명일 수 있음). README 가상환경 활성화는 Windows GitBash·Windows PowerShell·Linux/Mac별 안내
...
```json
{
  ...
  "code_examples": [
    {"lang":"python","code":"graph = StateGraph(State)...","explain":"State/Reducer로 상태 공유","source":"examples/langgraph/state_graph.py#build_state_graph#12"}
  ],
  "sources": [
